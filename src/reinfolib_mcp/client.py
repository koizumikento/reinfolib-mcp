...
class ReinfiolibClient:
            ...
            try:
                ...
                if response.status_code == 200:
                    try:
                        ...
                        raise DataFormatError(f"レスポンスのパース失敗: {e}")
                ...
                raise NetworkError(f"接続エラー: {e}")
            
            except httpx.TimeoutException as e:
                if retries < self.max_retries:
                    await asyncio.sleep(2 ** retries)  # Exponential backoff
                    return await self._make_request(endpoint, params, retries + 1)
                raise TimeoutError(f"リクエストタイムアウト: {e}")
            
            except Exception as e:
                raise ReinfiolibAPIError(f"予期しないエラー: {e}")
    ...
    ) -> Union[Dict[str, Any], RealEstateSearchResult]:
        """
...
class SyncReinfiolibClient:
    ...
    def search_real_estate_transactions(self, **kwargs) -> Union[Dict[str, Any], RealEstateSearchResult]:
        """不動産取引価格情報を検索します（同期版）"""
