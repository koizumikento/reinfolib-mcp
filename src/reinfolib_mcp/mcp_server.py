...
def create_mcp_server(api_key: str | None = None) -> FastMCP:
    ...
    async def search_real_estate_transactions(
        ...
        Args:
            year: 取引年（YYYY）
            area: 都道府県コード（area/city/stationのいずれか必須）
            city: 市区町村コード
            station: 駅コード
            quarter: 四半期（1-4）
    ...
    async def get_municipalities(area: str, language: str = "ja") -> dict[str, Any]:
        """
        指定都道府県の市区町村一覧を取得します。

        Args:
            area: 都道府県コード（01-47）例：東京都=13、京都府=26
            language: 言語（ja:日本語、en:英語）
        ...
        try:
            ...
            municipalities = await client.get_municipalities(area=area, lang=lang)
    ...
    async def get_real_estate_points(
        ...
        Args:
            ...
            tile_x: タイルX座標
            tile_y: タイルY座標
            from_period: 取引時期From（YYYYN）
            to_period: 取引時期To（YYYYN）
            response_format: レスポンス形式（geojson、pbf）
    ...
    @mcp.tool(
        ...
        description="現行の公開API 35種をAPI IDと公式パラメータ名で呼び出します。",
        tags={"api", "public-data", "geospatial"},
    )
    async def get_api_data(
        api_id: str,
        parameters: dict[str, Any],
    ) -> Any:
        """公開API一覧にある任意のAPIを呼び出します。

        Args:
            api_id: 公開API ID（例: XKT026）
    ...
    async def get_server_status() -> dict[str, Any]:
        ...
        return {
            ...
            "endpoints": {
                api_id: contract.title for api_id, contract in API_CONTRACTS.items()
            },
            "status": "healthy",
        }
...
def run_server(
    ...
    if not api_key:
        ...
        print(
            "環境変数REINFOLIB_API_KEYを設定するか、--api-keyオプションで指定してください"
        )
        return
